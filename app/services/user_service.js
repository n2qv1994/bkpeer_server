...
User_Services.register = function(user_info, callback) {
  
  user_helper.find_by_username(user_info.username)
  .then(function(message) {
    var user = message.data;
    if(!user) {
      user_helper.create(user_info)
      .then(function(message) {
        var new_user = message.data;
        return callback(false, {data: new_user, status_code: 200, status: "success"});
      })
